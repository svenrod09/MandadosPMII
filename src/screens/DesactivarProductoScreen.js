...
export default function DesactivarProductoScreen({ route, navigation }) {
    ...
    const btnPressed = async () => {
        if (!idproductos) {
            ...
            axios.put('http://192.168.0.24:5000/api/productos/eliminar?id='+ idproductos, { params: { id: idproductos } })
        ...
        <><Appbar.Header style={styles.colorPrimary}>
            ...
            <ScrollView>
                <Card style={styles.container} >
                    <Card.Cover source={{ uri: 'http://192.168.0.24:5000/producto/imgP/' + imagen }} />
                    <Card.Content>
                        <Title>Nombre del producto: {nombre}</Title>
                        <Paragraph>Precio: {telefono}</Paragraph>
                        <Paragraph>Cantidad: {direccion}</Paragraph>
