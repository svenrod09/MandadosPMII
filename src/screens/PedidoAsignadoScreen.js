...
export default function PedidoAsignadoScreen({ route, navigation }) {
    ...
    React.useEffect(() => {
        axios.get('http://192.168.0.11:5000/api/usuarios/listarEmpleados')
            .then((response) => {
    ...
    const btnPressed = async () => {
        if (!value) {
            ...
            axios.post("http://192.168.0.11:5000/api/detalle/guardar", {
                idpedido: idPedido,
                idempleado: value
            })
                .then((response) => {
                    axios.put("http://192.168.0.11:5000/api/pedido/asignarPedido?idPedido="+idPedido, {
                        idpedido: idPedido
                    })
