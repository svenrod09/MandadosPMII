...
export default function AgregarImagenScreen({ route, navigation }) {
    const [APIData, setAPIData] = React.useState([]);

    React.useEffect(() => {
        axios.get('http://192.168.1.9:5000/api/tienda/listar')
            .then((response) => {
    ...
    return (
        <><Appbar.Header style={styles.colorPrimary}>
            ...
            <ScrollView>
                <TouchableOpacity>
                    {APIData.map((element) => (
                        <Card key={element.idTienda} style={styles.container} onPress={() => navigation.navigate('SeleccionarImagenScreen', {
                            idTienda: element.idTienda
                        })}>
                            <Card.Cover source={{ uri: 'http://192.168.1.9:5000/tienda/img/' + element.imagen }} />
                            <Card.Content>
