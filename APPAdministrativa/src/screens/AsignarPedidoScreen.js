...
export default function AsignarPedidoScreen({ route, navigation }) {
    const [APIData, setAPIData] = React.useState([]);

    React.useEffect(() => {
        axios.get('http://192.168.1.9:5000/api/pedido/listarActivos')
            .then((response) => {
    ...
    if (!APIData) {
        Alert.alert("MANDADITOS", "No existen ningún pedido activo.");
        navigation.goBack();
        return null;
    }
    ...
    return (
        <><Appbar.Header style={styles.colorPrimary}>
            ...
            <Appbar.Content title="Asignar Pedidos" />
            <Appbar.Action icon="format-horizontal-align-left" onPress={() => navigation.replace("StartScreen")} />
        </Appbar.Header>
            <ScrollView>
                <TouchableOpacity>
                    {APIData.map((element) => (
                        <Card key={element.idPedido} style={styles.container} onPress={() => navigation.navigate('PedidoAsignadoScreen', {
