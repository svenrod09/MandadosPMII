...
export default function ModificarEmpScreen({ route, navigation }) {

    const [APIData, setAPIData] = React.useState([]);
    React.useEffect(() => {
        axios.get('http://192.168.1.9:5000/api/usuarios/ListarTodos')
            .then((response) => {
    ...
    return (
        <><Appbar.Header style={styles.colorPrimary}>
            ...
            <ScrollView>
                {APIData.map((element) => (
                     <List.Item onPress={() => navigation.navigate('ActualizarEmpScreen', {
