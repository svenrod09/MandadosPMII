...
export default function ConfirmarEntregaScreen({ route, navigation }) {
    ...
    React.useEffect(() => {
        axios.get('http://192.168.1.9:5000/api/pedido/listarAsignados', { params: {id:id} })
            .then((response) => {
    ...
    const btnPressed = async () => {
        if (!id) {
            ...
            axios.put("http://192.168.1.9:5000/api/detalle/modificar?id="+idDetalle, {
                idDetalle: idDetalle,
                activo: 1
            })
                .then((response) => {
                    axios.put("http://192.168.1.9:5000/api/pedido/entregarPedido?idPedido="+id, {
                        idpedido: idPedido
                    })
