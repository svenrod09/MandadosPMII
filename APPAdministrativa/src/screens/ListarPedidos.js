...
export default function ListarPedidos({ route, navigation }) {
    const [APIData, setAPIData] = React.useState([]);

    React.useEffect(() => {
        axios.get('http://192.168.1.9:5000/api/pedido/listar')
            .then((response) => {
    ...
    return (
        <><Appbar.Header style={styles.colorPrimary}>
            ...
            <ScrollView>
                <TouchableOpacity>
                    {APIData.map((element) => (
                        <Card key={element.idPedido} style={styles.container}>
                            ...
                            <Card.Content>
                                ...
                                <Paragraph>Estado: {element.estado}</Paragraph>
                            </Card.Content>
                        </Card>
                    ))}
                </TouchableOpacity>
